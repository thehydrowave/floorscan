...
export default function DemoClient() {
  ...
  return (
    <div className="min-h-screen bg-ink">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          {demoMode!==null&&(
            <div className="flex items-center gap-1 glass border border-white/10 rounded-xl p-1">
              {([{key:"ia",icon:BrainCircuit,label:d("sel_ia_title"),active:"bg-accent"},{key:"measure",icon:PenLine,label:d("sel_met_title"),active:"bg-accent"},{key:"chantier",icon:ClipboardList,label:"Chantier",active:"bg-orange-600"},{key:"facade",icon:Building2,label:d("sel_fa_title"),active:"bg-amber-600"},{key:"diff",icon:GitCompare,label:d("sel_di_title"),active:"bg-teal-600"},{key:"cartouche",icon:FileSearch,label:d("sel_ca_title"),active:"bg-violet-600"}] as const).map(({key,icon:Icon,label,active})=>(
          ...
          {demoMode===null&&(
            <motion.div key="select" initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-12}} transition={{duration:0.25}}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
                ...
                <button onClick={()=>setDemoMode("diff")} className="group relative text-left glass border border-white/10 rounded-3xl p-8 hover:border-teal-500/40 hover:bg-teal-500/5 transition-all duration-300 overflow-hidden">
                ...
                <button onClick={()=>setDemoMode("cartouche")} className="group relative text-left glass border border-white/10 rounded-3xl p-8 hover:border-violet-500/40 hover:bg-violet-500/5 transition-all duration-300 overflow-hidden">
      ...
      {demoMode&&demoMode!=="chantier"&&<ChatPanel result={analysisResult} currentStep={step} autoOpen={demoMode==="ia"||demoMode==="measure"}/>}
    </div>
  );
}
