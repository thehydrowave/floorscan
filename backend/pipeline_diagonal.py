"""
pipeline_diagonal.py — Pipeline H : Détection améliorée des murs inclinés
==========================================================================

Améliorations clés vs pipelines A-G :
  1. Morphologie adaptative : kernel elliptique au lieu de rectangulaire
  2. Hough Lines probabilistes à angles libres (pas seulement H/V)
  3. Squelettisation adaptative avec épaisseur variable par direction
  4. Reconstruction vectorielle par groupes angulaires
  5. Flood fill renforcé avec fermeture morphologique diagonale
"""
...
def detect_walls_diagonal(img_rgb: np.ndarray, min_wall_thickness: int = 3) -> np.ndarray:
    gray = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2GRAY)
    _, binary_thick = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    k_open_ellipse = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    walls_thick = cv2.morphologyEx(binary_thick, cv2.MORPH_OPEN, k_open_ellipse, iterations=1)
    blurred = cv2.GaussianBlur(gray, (3, 3), 0)
    edges = cv2.Canny(blurred, 30, 100, apertureSize=3)
    k_dilate = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2, 2))
    edges_dilated = cv2.dilate(edges, k_dilate, iterations=1)
    walls_combined = cv2.bitwise_or(walls_thick, edges_dilated)
    k_close_cross = cv2.getStructuringElement(cv2.MORPH_CROSS, (5, 5))
    walls_final = cv2.morphologyEx(walls_combined, cv2.MORPH_CLOSE, k_close_cross, iterations=2)
    k_clean = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    walls_final = cv2.morphologyEx(walls_final, cv2.MORPH_OPEN, k_clean, iterations=1)
    return walls_final
...
def extract_wall_lines_hough(walls_mask: np.ndarray, img_shape: tuple) -> list:
    ...
    lines = cv2.HoughLinesP(mask_dilated, rho=1, theta=np.pi/180, threshold=20, minLineLength=15, maxLineGap=8)
    segments = []
    if lines is not None:
        for line in lines:
            ...
            segments.append({
                ...
                "is_diagonal": not (angle_deg < 15 or angle_deg > 165 or 75 < angle_deg < 105),
            })
    return segments


def segment_rooms_diagonal(walls, m_doors, m_windows, building_cnt, H, W, ppm):
    ...
    b2 = cv2.dilate(b1, k_ellipse, iterations=1)
    interior = cv2.subtract(building, b2)
    ...
    for i in range(1, num_labels):
        ...
        rooms_raw.append({
            ...
            "bbox_norm": {"x": round(x/W, 4), "y": round(y/H, 4), "w": round(w/W, 4), "h": round(h/H, 4)},
            "area_m2": round(area_m2, 2) if area_m2 else None,
            "area_px2": area_px,
            "polygon_norm": [{"x": round(float(pt[0])/W, 5), "y": round(float(pt[1])/H, 5)} for pt in cnt_i.reshape(-1, 2).tolist()],
        })
    ...
    for r in rooms_raw:
        ...
        label_map = {"living room": ("Séjour", f"Séjour {n}"), "bedroom": ("Chambre", f"Chambre {n}"), "bathroom": ("Salle de bain", f"SDB {n}"), "hallway": ("Couloir", f"Couloir {n}"), "kitchen": ("Cuisine", f"Cuisine {n}")}
        if lbl in label_map:
            label_fr = label_map[lbl][0] if n == 1 else label_map[lbl][1]
        else:
            label_fr = f"Pièce {n}"
...
def reconstruct_walls_from_lines(segments, H, W, line_thickness=4):
    mask = np.zeros((H, W), np.uint8)
    for seg in segments:
        cv2.line(mask, (seg["x1"], seg["y1"]), (seg["x2"], seg["y2"]), 255, line_thickness, cv2.LINE_AA)
    k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, k, iterations=1)
    return mask


def run_pipeline_h(img_rgb, img_pil, client, ppm, cfg):
    import time
    ...
    try:
        ...
        ri, legend, md1, mw1, mwall1, rows1, _ = pip.infer_pass(img_pil, client, model_id, cfg["pass1_tile"], cfg["pass1_over"], write_rooms=True, conf_min_door=cfg["conf_min_door"], conf_min_win=cfg["conf_min_win"], cfg=cfg)
        ...
        _, _, md2, mw2, _, rows2, _ = pip.infer_pass(img_pil, client, model_id, cfg["pass2_tile"], cfg["pass2_over"], write_rooms=False, conf_min_door=cfg["conf_min_door"], conf_min_win=cfg["conf_min_win"], cfg=cfg)
        ...
        m_walls_diagonal = detect_walls_diagonal(img_rgb, min_wall_thickness=3)
        walls_boundary = pip._walls_from_rooms_index(ri, H, W)
        m_walls = cv2.bitwise_or(walls_boundary, m_walls_diagonal) if cv2.countNonZero(walls_boundary) > 0 else m_walls_diagonal

        segments = extract_wall_lines_hough(m_walls, (H, W))
        wall_segments_all = segments
        if segments:
            m_walls_reconstructed = reconstruct_walls_from_lines(segments, H, W, line_thickness=3)
            m_walls = cv2.bitwise_or(m_walls, m_walls_reconstructed)
        ...
        if cnt is not None:
            ...
            r_px = max(1, int(round((cfg.get("wall_thickness_m", 0.20) * ppm) / 2.0))) if ppm else max(1, cfg.get("wall_thickness_px_fallback", 10) // 2)
        ...
        mask_doors_b64 = pip._np_to_b64(pip._mask_to_rgba(m_doors, (217, 70, 239), 90)) if cv2.countNonZero(m_doors) > 0 else None
        mask_windows_b64 = pip._np_to_b64(pip._mask_to_rgba(m_wins, (34, 211, 238), 90)) if cv2.countNonZero(m_wins) > 0 else None
        mask_walls_b64 = pip._np_to_b64(pip._mask_to_rgba(m_walls, (96, 165, 250), 90)) if cv2.countNonZero(m_walls) > 0 else None
        ...
        if diagonal_segments:
            for seg in diagonal_segments:
                cv2.line(mask_diagonal_walls, (seg["x1"], seg["y1"]), (seg["x2"], seg["y2"]), 255, 3, cv2.LINE_AA)
        mask_diagonal_b64 = pip._np_to_b64(pip._mask_to_rgba(mask_diagonal_walls, (249, 115, 22), 200)) if cv2.countNonZero(mask_diagonal_walls) > 0 else None

    except Exception as e:
        error = str(e)
        logger.error("Pipeline H failed: %s", e, exc_info=True)
        doors_count = 0; windows_count = 0; diagonal_pct = 0.0
    ...
    wall_segments_json = [{"x1_norm": s["x1_norm"], "y1_norm": s["y1_norm"], "x2_norm": s["x2_norm"], "y2_norm": s["y2_norm"], "length_m": round(s["length_px"]/ppm, 2) if ppm else None, "angle_deg": s["angle_deg"], "is_diagonal": s["is_diagonal"]} for s in wall_segments_all]

    return {
        "id": "H", "name": "Diagonal (H)", "color": "#06B6D4",
        "description": "Morphologie adaptative + Hough multi-angles → meilleure détection des murs inclinés",
